...
export const getTradingAnalysisPrompt = (lang: Language, currentTime: string) => `
...
Tu raisonnes comme un desk institutionnel, pas comme un trader retail.
...
## LANGUE DE SORTIE
Tout le contenu textuel du JSON doit être en : ${lang === "fr" ? "FRANÇAIS" : "ENGLISH"}

---

## LOGIQUE TEMPORELLE (CRUCIAL)
L’heure actuelle de l’utilisateur est : ${currentTime}
Toutes tes estimations d’expiration de setups DOIVENT se baser sur ce point de référence.
Ne jamais inventer une date de départ différente.
...
Pour chaque setup (A et B), tu dois définir une date et heure d’expiration.
- Si intraday / scalping : expiration à +4h ou fin de session
- Si swing : expiration à +24h ou +48h
- Format strict : YYYY-MM-DD HH:mm
- Toujours basé sur l’heure actuelle fournie : ${currentTime}

---

## FORMAT DE SORTIE (OBLIGATOIRE)
AUCUN TEXTE en dehors du JSON.
Utilise des strings pour les prix afin de conserver le format exact (ex: "1.0500").

JSON attendu :
{
  "asset_class": "Forex | Crypto | Indices | Commodities | Stocks | Unknown",
  "market_bias": { "direction": "BUY | SELL", "confidence": 0 },
  "liquidity_zones": [
    { "type": "BUYSIDE | SELLSIDE", "price_approx": "0.0000", "strength": "low | medium | high", "reason": "" }
  ],
  ...
  "invalidation_rules": { "bias_invalidation": "", "setup_invalidation": "" },
  "limitations": []
}
`;
