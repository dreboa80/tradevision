...
import { AnalysisResponse, HistoryItem, SubscriptionStatus, PlanType } from './types';
...
const App: React.FC = () => {
    ...
    if (storedSub) {
      ...
      if (parsedSub.plan === 'silver') {
        ...
        if (now - parsedSub.startDate > thirtyDaysMs) {
            setIsExpired(true);
            setSubscription(null); // Revoke access locally, prompt pricing
        }
      }
    }
  }, []);
  ...
  const handleSelectPlan = (plan: PlanType) => {
    ...
    if (plan === 'silver') {
        ...
        // In a real app, this would be a callback from the PayPal SDK
        const newSub: SubscriptionStatus = {
            plan: 'gold',
            startDate: Date.now(),
            expiryDate: Date.now() + (30 * 24 * 60 * 60 * 1000) // 30 days renewable
        };
        localStorage.setItem('subscription_status', JSON.stringify(newSub));
        setSubscription(newSub);
        setIsExpired(false);
                ...
                {!file && !result && (
                  <div className="flex flex-col items-center min-h-[calc(100vh-100px)] px-4 py-12">
                    ...
                    >
                        ...
                        <div className="inline-block px-4 py-1 rounded-full bg-institutional-border/30 text-xs font-mono text-institutional-muted">
                            {t.supports}
                        </div>
                        <input 
                            type="file" 
                    ...
                    <div className="mt-16 text-center space-y-2">
                         <p className="text-[10px] uppercase tracking-widest text-institutional-muted">{t.powered_by}</p>
                         <p className="font-bold text-institutional-accent tracking-tight">MoneySpaces</p>
                    </div>
                  </div>
                )}

                {/* Selected File & Analysis Flow */}
                {file && !result && (
                    <div className="max-w-4xl mx-auto mt-12 px-6">
                        <div className="bg-institutional-card border border-institutional-border rounded-xl overflow-hidden shadow-2xl">
                            <div className="relative aspect-video bg-black/50 flex items-center justify-center group">
                                ...
                                <button 
                                    onClick={reset}
                                    className="absolute top-4 right-4 bg-black/60 hover:bg-black/80 text-white px-3 py-1 rounded-md text-xs font-mono backdrop-blur border border-white/10"
                                >
                                    {t.cancel}
                                </button>
                            </div>
                                ...
                                {!isAnalyzing ? (
                                    <div className="flex flex-col items-center space-y-6">
                                        <div className="text-center">
                                            <h3 className="text-lg font-medium text-white">{t.ready_to_analyze}</h3>
                                            <p className="text-sm text-institutional-muted font-mono mt-1">{file.name}</p>
                                        </div>
                                        <button 
                                            onClick={triggerAnalysis}
                                            className="bg-institutional-accent hover:bg-emerald-400 text-black font-bold py-3 px-8 rounded-lg transition-all transform hover:scale-105 shadow-[0_0_20px_rgba(0,220,130,0.3)] flex items-center gap-2"
                                        >
                                            <IconUpload size={20} />
                                            {t.run_engine}
                                        </button>
                ...
                {result && (
                    <div className="space-y-6">
                         <div className="max-w-7xl mx-auto px-6 pt-6 flex justify-between items-center">
                            <button 
                                onClick={reset}
                                className="text-xs font-mono text-institutional-muted hover:text-white flex items-center gap-2"
                            >
                                ← {t.new_analysis}
                            </button>
                         </div>
                        <Dashboard data={result} lang={lang} />
                    </div>
                )}
            </>
        )}
